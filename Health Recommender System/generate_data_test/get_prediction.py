...
#helper functions and class for depression (DO NOT TOUCH)
...
# scan the DynamoDB table and get each item
...
for item in items:
    ...
    for ele in item:
     ...
     #print(item[ele])
    df = pd.DataFrame([valuelist], columns=attrlist)
    #call diabetes
    diaPred= diabetesPredict(df)
    #call heart
    heartPred= heartDiseasePredict(df)
    isDiabetes=diaPred[0][1]
    isheartDisease=0
    #add to DB
    item['isDiabetes'] = {'N': str(isDiabetes)}
    if heartPred[0]=='Presence':
        isheartDisease=1
    item['isHeartDisease']={'N':str(isheartDisease)}
    s3_url = item['mentalhealthdata']['S']
    print(s3_url)
    response = urllib.request.urlopen(s3_url)
    data = response.read()
    print(type(data))
    with open('/home/ubuntu/data/temp/mentalhealth/test.csv','wb') as f:
     f.write(data)
    path='/home/ubuntu/data/temp/mentalhealth/'
    depressPred = depressionPredition(path)
    item['isDepression']= {'N': str(depressPred)}
    s3_url = item['malariadata']['S']
    print(s3_url)
    response = urllib.request.urlopen(s3_url)
    data = response.read()
    print(type(data))
    with open('/home/ubuntu/data/temp/malaria/ml/test.png','wb') as f:
     f.write(data)
    input_dir_malaria = '/home/ubuntu/data/temp/malaria/'
    ...
    item['isMalaria']={'N':str(probMalaria[0])}
    os.remove('/home/ubuntu/data/temp/malaria/ml/test.png')
    s3_url = item['coviddata']['S']
    print(s3_url)
    response = urllib.request.urlopen(s3_url)
    data = response.read()
    print(type(data))
    with open('/home/ubuntu/data/temp/covid/ml/test.jpeg','wb') as f:
     f.write(data)
    input_dir_covid = '/home/ubuntu/data/temp/covid/'
    ...
    alist=list(pred[0])
    item['isCovid']={'N':str(alist[0])}
    item['isPneuomonoa']={'N':str(alist[2])}
